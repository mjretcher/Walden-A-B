...
function unitTitle(units: Unit[]) { return units.length === 4 ? "All Units" : units.map((unit) => UNIT_LABEL[unit].replace("Unit ", "")).join(" & "); }
...
export default async function AbMenuReport({ searchParams }: { searchParams?: Promise<MenuSearchParams> }) {
  ...
  const filteredOfferings = offerings.filter((offering) => { const eligibleUnits = readStringArray(offering.eligibleUnits); return !eligibleUnits.length || units.some((unit) => eligibleUnits.includes(unit)); });
  const areaNames = Array.from(new Set(filteredOfferings.map((offering) => offering.area.name))).sort(sortAreas);

  return (
    <AppShell user={user}>
      <style>{`@media print { @page { size: letter landscape; margin: 0.14in; } .ab-menu-report{display:block!important;gap:0!important}.ab-menu-sheet{background:white!important;break-after:page!important;box-shadow:none!important;margin:0!important;max-width:none!important;min-height:0!important;transform:none!important;width:147%!important;zoom:.68}.ab-menu-sheet:last-child{break-after:auto!important}.ab-menu-sheet__header{margin:0 auto .02in!important;max-width:68%!important}.ab-menu-sheet__header span{font-size:11px!important;line-height:1!important;padding:1px 5px!important}.ab-menu-sheet__grid{border-left-width:1px!important;border-top-width:1px!important}.ab-menu-sheet__period-heading{border-bottom-width:1px!important;border-right-width:1px!important;font-size:10px!important;line-height:1!important;padding:2px 3px!important}.ab-menu-sheet__period-heading small{display:block!important;font-size:6.6px!important;line-height:.95!important;margin-top:1px!important}.ab-menu-sheet__cell{border-bottom-width:1px!important;border-right-width:1px!important;min-height:0!important;padding:.016in .024in!important}.ab-menu-sheet__cell h2{font-size:9px!important;font-weight:700!important;line-height:.95!important;margin:0 0 1px!important}.ab-menu-sheet__cell li{break-inside:avoid!important;font-size:7px!important;line-height:.9!important;margin:0!important;padding-left:5px!important}.ab-menu-sheet__cell li::before{left:0!important}.ab-menu-sheet__cell li>span{font-weight:800!important}.ab-menu-sheet__cell strong{font-size:6.5px!important;margin-left:1px!important;white-space:nowrap!important}.ab-menu-sheet__cell em{display:inline!important;font-size:5.6px!important;line-height:.9!important;margin:0 0 0 2px!important;padding-left:0!important}.ab-menu-sheet__area-total{font-size:6.4px!important;line-height:.95!important;margin:1px 0 0!important}.ab-menu-sheet__footer{font-size:6.5px!important;line-height:.95!important;min-height:0!important;padding:.018in .1in!important}.ab-menu-sheet__footer p{margin:0 0 1px!important}.ab-menu-sheet__area-rollups{display:grid!important;gap:1px 5px!important;grid-template-columns:repeat(4,minmax(0,1fr))!important;margin:0 0 1px!important} }`}</style>
      <div className="no-print mb-5 flex flex-wrap items-center justify-between gap-4"><div><h1 className="text-3xl font-black text-forest-900">Printable A/B Menu</h1><p className="mt-1 text-slate-600">Paper-style menu: A Day and B Day print as separate pages.</p></div><div className="flex flex-wrap gap-2"><Link className={secondaryButtonClass} href="/admin/menu-builder">Menu Builder</Link><PrintButton label="Print menu" /></div></div>
      <form className="no-print mb-5 grid gap-4 rounded-xl border border-slate-200 bg-white p-4 shadow-soft lg:grid-cols-3" method="get"><fieldset><legend className="mb-2 text-sm font-black text-forest-900">Units</legend><div className="flex flex-wrap gap-2">{(Object.values(Unit) as Unit[]).map((unit) => <label key={unit} className="cursor-pointer"><input className="peer sr-only" defaultChecked={units.includes(unit)} name="unit" type="checkbox" value={unit} /><span className="inline-flex rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-black peer-checked:border-forest-700 peer-checked:bg-forest-700 peer-checked:text-white">{UNIT_LABEL[unit]}</span></label>)}</div></fieldset><label className="grid gap-2 text-sm font-black text-forest-900">Registration window<select className="min-h-11 rounded-lg border border-slate-200 px-3" name="window" defaultValue={registrationWindow}>{Object.values(RegistrationWindow).map((window) => <option key={window} value={window}>{REGISTRATION_WINDOW_LABEL[window]}</option>)}</select></label><fieldset><legend className="mb-2 text-sm font-black text-forest-900">Print details</legend><div className="flex flex-wrap gap-2"><label className="cursor-pointer"><input className="peer sr-only" defaultChecked={showCounts} name="counts" type="checkbox" value="show" /><span className="inline-flex rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-black peer-checked:border-lake-700 peer-checked:bg-lake-700 peer-checked:text-white">Show class and total counts</span></label><label className="cursor-pointer"><input className="peer sr-only" defaultChecked={showNotes} name="notes" type="checkbox" value="show" /><span className="inline-flex rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-black peer-checked:border-lake-700 peer-checked:bg-lake-700 peer-checked:text-white">Show notes</span></label><label className="cursor-pointer"><input className="peer sr-only" defaultChecked={showUnitLabels} name="unitLabels" type="checkbox" value="show" /><span className="inline-flex rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-black peer-checked:border-lake-700 peer-checked:bg-lake-700 peer-checked:text-white">Show units by counts</span></label></div></fieldset><div className="flex flex-wrap gap-2 lg:col-span-3"><button className="rounded-md bg-forest-800 px-4 py-2 text-sm font-semibold text-white" type="submit">Update menu</button><a className={secondaryButtonClass} href="/reports/ab-menu">Reset</a></div></form>
