...
import { importRealCampers, previewRealCamperImport } from "@/lib/real-data-import";
...
export async function POST(request: NextRequest) {
  ...
  const preview = await previewRealCamperImport(prisma, text);

  if (!commit) return NextResponse.json({ preview });

  const result = await importRealCampers(prisma, text, { replaceSamples: replaceSamples === true });
