...
export async function decideSwitch(formData: FormData) {
  ...
  if (request.type === SwitchType.CAMPER) {
    if (!request.camper || !request.requestedOffering) throw new Error("Camper switch is incomplete.");
    if (user.role === UserRole.AREA_HEAD && user.areaId && user.areaId !== request.requestedOffering.areaId) {
      throw new Error("Area Heads may only approve switches into their own area.");
    }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.registration.create({
        data: {
          camperId: request.camperId!,
          offeringId: request.requestedOfferingId!,
          sessionId: request.sessionId,
          menuId: request.requestedOffering.menuId,
          period: request.period,
    ...
    if (!request.staffAssignment || !request.requestedOfferingId) throw new Error("Staff switch is incomplete.");
    await prisma.$transaction(async (tx) => {
      await tx.staffAssignment.update({
        where: { id: request.staffAssignmentId! },
        data: { offeringId: request.requestedOfferingId!, period: request.period, notes: "Updated through staff switch workflow." }
      });
      await tx.switchRequest.update({
        where: { id },
